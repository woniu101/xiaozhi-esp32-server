...
class StreamingPcmTranscoder:
    """Incrementally resample signed 16-bit mono PCM for the active device."""
    ...
    def finish(self) -> None:
        # Odd trailing bytes cannot form an int16 PCM sample.
        self._source_tail = b""
        if self.output_format == "pcm":
            if self._pcm_buffer:
...
class SingleStreamTTSMixin:
    """Queue, cancellation and PCM transport shared by single HTTP streams."""
    ...
    def synthesis_cancelled(self) -> bool:
        return self._synthesis_cancel_event.is_set()
    ...
    def reset_stream_for_fallback(self) -> None:
        """Discard an unplayed partial stream before replaying the segment as WAV."""
        self._stream_transcoder = None
        self._stream_segment_announced = False
        self._stream_packet_count = 0
    ...
    def feed_wav_to_stream(self, audio: bytes) -> int:
        ...
        try:
            ...
            raise RuntimeError("TTS 降级响应不是有效 WAV 音频") from exc
        if source_rate <= 0 or frame_count <= 0 or not pcm:
            raise RuntimeError("TTS 降级响应不包含有效音频帧")
        if channels == 2:
